...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument("--list-voices", action="store_true",
    ...
    parser.add_argument("-o", "--output", type=Path, default="output.wav",
    ...
    parser.add_argument("--output-device", type=int,
                        help="Output device to use.")
    parser.add_argument("--language-code", default='en-US',
                        help="A language of input text.")
    parser.add_argument(
        "--sample-rate-hz", type=int, default=44100, help="Number of audio frames per second in synthesized audio."
    )
    parser.add_argument("--encoding", default="LINEAR_PCM",
                        choices={"LINEAR_PCM", "OGGOPUS"}, help="Output audio encoding.")
    parser.add_argument("--custom-dictionary", type=str,
...
def main() -> None:
    ...
    auth = riva.client.Auth(ssl_cert=args.ssl_cert, use_ssl=True,
                            uri="grpc.nvcf.nvidia.com:443", metadata_args=args.metadata)
    ...
    if args.list_voices:
        ...
        for model_config in config_response.model_config:
            ...
            subvoices = [voice.split(
                ':')[0] for voice in model_config.parameters['subvoices'].split(',')]
            full_voice_names = [voice_name + "." +
                                subvoice for subvoice in subvoices]
